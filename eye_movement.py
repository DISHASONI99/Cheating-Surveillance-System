...
# print(os.path.exists("model/shape_predictor_68_face_landmarks.dat"))  # Should return True
    ...
    for face in faces:
        ...
        if left_pupil and right_pupil:
            ...
            if lx < eye_width // 3 and rx < eye_width // 3:
                gaze_direction = "Looking Left(Cheating)"
            elif lx > 2 * eye_width // 3 and rx > 2 * eye_width // 3:
                gaze_direction = "Looking Right(Cheating)"
            elif norm_ly < 0.3 and norm_ry < 0.3:
                gaze_direction = "Looking Up(Cheating)"
...
#     processed_frame, gaze = process_eye_movement(frame)
#     cv2.putText(processed_frame, gaze, (50, 50), cv2.FONT_HERSHEY_SIMPLEX, 1, (255, 0, 0), 2)
